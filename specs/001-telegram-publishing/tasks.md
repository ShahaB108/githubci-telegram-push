...
- [ ] T001 Create project scaffolding per plan.md: `pipeline/__init__.py`, `tests/unit/`,
      `tests/contract/`, `tests/integration/` (each with empty `__init__.py` or as pytest
      rootdir dirs), `posts/.gitkeep`, `delivery/.gitkeep`
- [ ] T002 [P] Add dependency manifests: `requirements.txt` (PyYAML, Markdown, requests - the
      three direct dependencies from plan.md) and `requirements-dev.txt` (pytest, ruff)
- [ ] T003 [P] Configure pytest in `pytest.ini`: testpaths `tests`, python_files `test_*.py`,
      addopts `-q`

## Phase 2: Foundational (Blocking Prerequisites)

- [ ] T004 [P] Implement environment configuration in `pipeline/config.py` per
      ...
      the environment
- [ ] T005 [P] Implement ledger store in `pipeline/ledger.py` per data-model.md: schema
...
- [ ] T006 [P] [US1] Write failing unit tests for Markdown conversion in
...
- [ ] T007 [P] [US1] Write failing unit tests for message splitting in
...
- [ ] T008 [P] [US1] Write failing unit tests for detection in `tests/unit/test_detect.py`:
...
- [ ] T009 [P] [US1] Write failing contract tests in `tests/contract/test_publish_cli.py`:
...
- [ ] T010 [P] [US1] Implement `pipeline/convert.py`: Markdown to Telegram HTML conversion
      and 4096-character block-boundary splitting with part markers (satisfies T006, T007)
- [ ] T011 [P] [US1] Implement `pipeline/detect.py`: scan `PIPELINE_POSTS_DIR` for `.md`
...
- [ ] T012 [US1] Implement `pipeline/deliver.py`: send each part via Telegram sendMessage
...
- [ ] T013 [US1] Implement `pipeline/main.py` orchestration: detect -> convert -> deliver ->
...
- [ ] T014 [P] [US2] Write failing unit tests for bounded retry in `tests/unit/test_retry.py`:
...
- [ ] T015 [P] [US2] Write failing contract tests for failure modes in
...
- [ ] T016 [P] [US2] Add bounded retry with backoff and `Retry-After` handling to
      `pipeline/deliver.py` (satisfies T014; depends on T012)
- [ ] T017 [US2] Implement failure paths in `pipeline/main.py` and `pipeline/config.py`:
...
- [ ] T018 [P] [US3] Write failing unit tests for idempotency in
...
- [ ] T019 [P] [US3] Write failing contract tests in `tests/contract/test_ledger_commit.py`:
...
- [ ] T020 [US3] Implement ledger commit-back, delivered-post skip, and partial-resume logic
...
- [ ] T021 [P] Create `.github/workflows/publish.yml`: trigger on push to `main` touching
...
- [ ] T022 [P] Create `README.md` publishing section: how to add a post, required secrets and
...
- [ ] T023 [P] Add `ruff.toml` configuration and fix all lint findings in `pipeline/` and
      `tests/` (constitution Principle IV)
- [ ] T024 Run the full pytest suite and ruff until green, then walk the quickstart.md
